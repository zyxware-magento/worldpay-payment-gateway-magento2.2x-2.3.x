 ...
 * Reading xml
 */
class StateResponse implements \Zyxware\Worldpay\Model\Payment\State
{
    private $orderCode;
    ...
    public static function createFromCancelledResponse($params)
    {
        ...
        $orderCode = \Zyxware\Worldpay\Model\Payment\StateResponse::_extractOrderCode($orderkey);
        $merchantCode = \Zyxware\Worldpay\Model\Payment\StateResponse::_extractMerchantCode($orderkey);

        return new self(
            $orderCode,
            $merchantCode,
            \Zyxware\Worldpay\Model\Payment\State::STATUS_CANCELLED,
            null
        );
    }
    ...
    public static function createFromPendingResponse($params, $paymentType = null)
    {
        ...
        if(!empty($paymentType) && $paymentType == "KLARNA-SSL"){
            ...
            return new self(
                $orderCode,
                $merchantCode,
                \Zyxware\Worldpay\Model\Payment\State::STATUS_PENDING_PAYMENT,
                null
            );
        }
    }
